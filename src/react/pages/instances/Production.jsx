...
const Production = () => {
				...
				{(startDate || pressDate || endDate) && (
					<InstanceFacet labelText="Dates">
						{startDate && (
							<div>
								<b>Starts:</b>
								{` ${formatDate(startDate, dateFormatOptions)}`}
							</div>
						)}
						...
						{pressDate && (
							<div>
								<b>Press:</b>
								{` ${formatDate(pressDate, dateFormatOptions)}`}
							</div>
						)}
						...
						{endDate && (
							<div>
								<b>Ends:</b>
								{` ${formatDate(endDate, dateFormatOptions)}`}
							</div>
						)}
					</InstanceFacet>
				)}
					...
					<InstanceFacet labelText="Part of">
						<div className="nested-instance">
							<InstanceFacet labelText="Production">
								...
								{surProduction.subtitle && <p>{surProduction.subtitle}</p>}
							</InstanceFacet>
						...
						{subProductions.map((subProduction, index) => (
							<div key={index} className="nested-instance">
								<InstanceFacet labelText="Production">
									...
									{subProduction.subtitle && <p>{subProduction.subtitle}</p>}
								</InstanceFacet>
						...
						<ListWrapper>
							{cast.map((castMember, index) => (
								<li key={index}>
									...
									{castMember.roles?.length > 0 && <AppendedRoles roles={castMember.roles} />}
								</li>
							))}
						</ListWrapper>
					</InstanceFacet>
						...
						<ListWrapper>
							{reviews.map((review, index) => (
								<li key={index}>
									...
									{review.date && <AppendedDate date={review.date} />}
																...
																<Fragment key={index}>
																	...
																	{nomination.recipientProductions.length > 0 && (
																		<>
																			...
																			<CommaSeparatedProductions
																				productions={
																					nomination.recipientProductions
																				}
																			/>
																			<>{')'}</>
																		</>
	...
	return <InstancePageWrapper instance={production}>{renderProduction(production)}</InstancePageWrapper>;
};
