...
const ProductionLinkWithContext = (props) => {
	...
	return (
		<>
			...
			{production.surProduction && <PrependedSurInstance surInstance={production.surProduction} />}

			<InstanceLink instance={production} />

			{production.venue && <AppendedVenue venue={production.venue} />}

			{production.subVenue && <AppendedVenue venue={production.subVenue} />}

			{(production.startDate || production.endDate) && (
				<AppendedProductionDates startDate={production.startDate} endDate={production.endDate} />
			)}
		</>
	);
};
