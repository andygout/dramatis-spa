		...
		context('resolves with data', () => {
			...
			it('will call renderJson module', done => {
				...
				createInstance(method, methodStub).then(() => {
					...
					expect(stubs.renderJson.calledWithExactly(req, res, responseFixture.theatre)).to.be.true;
	...
	describe('list method', () => {
		...
		context('resolves with data', () => {
			...
			it('will call renderJson module', done => {
				...
				createInstance(method, methodStub).then(() => {
					expect(stubs.renderJson.calledOnce).to.be.true;
					expect(stubs.renderJson.calledWithExactly(req, res, responseListFixture.theatres)).to.be.true;
					expect(next.notCalled).to.be.true;
					done();
				});
